# Episode 2: Work IQ: A2A for Context‑Aware, Agentic Experiences

[![Episode 2 video](../../images/work-iq/ep2.png)](https://aka.ms/iq-series/work-iq/episode2)

📅 **June 2, 2026 at 9 AM PT** | 📺 [Watch the session](https://aka.ms/iq-series/work-iq/episode2) | 📂 [Try the lab](./lab/)


## 🎥 Session Summary

### 🎬 Executive Introduction

Vanessa Feliberti Bautista introduces Work IQ and its A2A protocol support.

### 💬 Tech Talk

Ayça Baş and Darrel Miller walk through the idea of the A2A protocol for context-aware agentic experiences, explaining:

- The A2A (Agent-2-Agent) protocol
- How A2A discovery works
- Prototyping with Work IQ A2A

### 🖍 Doodle Summary

A visual summary of key takeaways by Tomomi Imura, showing how A2A fits in the Work IQ architecture.

![Doodle summary Episode 2](../../images/work-iq/E2-recap.png)

## � Try the Lab

Ready to get hands-on? Head to the [Episode 2 lab](./lab/) for prerequisites, deployment instructions, and a step-by-step guidance.

## 🔗 Learn More

- 📖 [Work IQ overview](https://learn.microsoft.com/en-us/microsoft-365/copilot/extensibility/work-iq)
- 📚 [Copilot Dev Camp - Work IQ](https://microsoft.github.io/copilot-camp/pages/work-iq/)

## 💬 Community

- Ask your questions on our [Discord channel](https://discord.gg/REmjGvvFpW)

### 🚀 Next Up: Continue to [Episode 3](../3-Work-IQ-Tooling-with-MCP-and-Copilot-CLI/) for the next step—Tooling with MCP & Copilot CLI!
